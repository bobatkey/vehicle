...
runTypeChecking prog = do
  ...
  let prog4 = evalStateT prog3 mempty
...
getDeclCtx = lift (lift get)

addToDeclCtx :: WithProvenance Identifier -> CheckedExpr -> TCM ()
addToDeclCtx ident t' =
  lift $ lift $ modify (Map.insert (deProv ident) t')
...
freshMeta :: Provenance -> CheckedExpr -> TCM (Meta, CheckedExpr, CheckedExpr)
freshMeta p resultType = do
  ...
  let metaType = foldr (tPiInternal Explicit) resultType (reverse (fmap snd boundCtx))
  ...
  let boundEnv =
        reverse [ Var (makeTypeAnn varType) (Bound varIndex)
...
insertImplicits :: Provenance -> CheckedExpr -> CheckedExpr -> TCM (CheckedExpr, CheckedExpr)
...
assertIsType :: Provenance -> CheckedExpr -> TCM ()
assertIsType _ (Type _) = return ()
assertIsType p e        = throwError $ Mismatch p e (Type 0) -- TODO: add a new TypingError 'ExpectedAType'
...
inferDecl = \case
  ...
  DeclNetw p ident t      -> do
    ...
    assertIsType p ty'
    addToDeclCtx ident t'
    return $ DeclNetw p ident t'
  DeclData p ident t      -> do
    (t', ty') <- infer t
    assertIsType p ty'
    addToDeclCtx ident t'
    return $ DeclData p ident t'
